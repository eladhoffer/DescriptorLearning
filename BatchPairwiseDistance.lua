...
local BatchPairwiseDistance, parent = torch.class('nn.BatchPairwiseDistance', 'nn.Module')

function BatchPairwiseDistance:__init(p)
   parent.__init(self)
...
function BatchPairwiseDistance:updateOutput(input)
...
function BatchPairwiseDistance:updateGradInput(input, gradOutput)
   if input[1]:dim() > 2 then
      error('input must be vector or matrix')
   end
   ...
   if input[1]:dim() == 1 then
      ...
      self.grad:resize(input[1]:size()):zero():typeAs(input[1])
      self.ones:resize(input[1]:size(2)):fill(1):typeAs(input[1])

      self.grad:addr(gradOutput, self.ones)
      self.gradInput[1]:cmul(self.grad:typeAs(input[1]))
   end
...
BatchPairwiseDistance._parent_type = parent.type

-- Fix the bug where tmp = nn.BatchPairwiseDistance:cuda() fails to convert table
...
function BatchPairwiseDistance:type(type)
